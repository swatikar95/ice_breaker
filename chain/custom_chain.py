...
from output_parser import summary_parser,ice_breaker_parser,topics_of_interest

llm = ChatOpenAI(temperature=0,model="gpt-3.5-turbo")
llm_creative = ChatOpenAI(temperature=1,model="gpt-3.5-turbo")

def get_summary_chain()->LLMChain:
    summary_template = """
    given the information about a person from linkedin{information} I want 
    you to create:
    1. a short summary
...
def get_interests_chain()->LLMChain:
    ...
    interesting_facts_prompt_template= PromptTemplate(
        ...
        partial_variables={
            "format_instructions":topics_of_interest.get_format_instructions()
        },
        )
...
def get_ice_breaker_chain()->LLMChain:
    ...
    return LLMChain(llm=llm,prompt=ice_breaker_prompt_template)
